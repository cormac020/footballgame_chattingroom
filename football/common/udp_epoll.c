...
int check_online(struct LogRequest *request){
    for(int i = 0; i < MAX; i++){
        ...
        if(bteam[i].online && !strcmp(request->name, rteam[i].name)) return 1;
    }
}
...
int udp_accept(int fd, struct User *user){
	...
	if(check_online(&request)){
		response.type = 1;
		strcpy(response.msg, "You already login!");
		sendto(fd, (void *)&response, sizeof(response), 0, (struct sockaddr *)&client, len);
		return -1;
	}

	response.type = 0;
	strcpy(response.msg, "Login Success. Enjot Yourself!");
	sendto(fd, (void *)&response, sizeof(response), 0, (struct sockaddr *)&client, len);
...
int list_online(char list[][20]){
    ...
    for (int i = 0; i < MAX; i++){
        if(rteam[i].online){
            strcpy(list[nuser++], rteam[i].name);
        }
        if(bteam[i].online){
            strcpy(list[nuser++], bteam[i].name);
        }
    }
    return nuser;
}
